...
There are quite some programs that export peptides with metadata based on spectral data. Both database matching and _de novo_ software. These files can be loaded using the button #button[Load identified peptides file] or by dragging in a file from the file explorer. Any number of files can be loaded at the same time. See the table below for supported formats. Opened identified peptides files can be closed with #button[Close file] in the peptides details pane.
