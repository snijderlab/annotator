...
The theoretical spectrum can be shown below the x axis. Displaying the unassigned peaks can be toggled. The colouration can be set to the ion type (default), to the peptide, the peptidoform, or grey (none). Also there is an option to remove all distance labels.
